...
const ProductCard: React.FC<ProductCardProps> = ({ image, title, price }) => (
  <>
    ...
    <div className="hidden md:flex app-bg rounded-lg p-4 w-67.25 h-103.5 flex-col card-shadow">
      ...
      <div className="mt-3">
        <h3 className="text-lg lg:text-xl satoshi uppercase">{title}</h3>
        <p className="text-xl lg:text-2xl satoshi-bold mt-5">${price.toFixed(2)}</p>
      </div>
    </div>
    {/* Mobile */}
    <div className="flex flex-col  mt-2 mb-5 md:hidden">
      <img
        src={image}
        alt={title}
        className="w-89.25 h-96 object-cover"
      />
      <div className="flex justify-between items-center text-base sm:text-lg mt-3 px-1 w-full">
